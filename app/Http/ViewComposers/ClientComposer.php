...
class ClientComposer
{
    ...
    */
    public function compose(View $view)
    {
        $user=Auth::user();
        //$user=Auth::user();
        // $jobsall=jobs::select('*')->whereNotIn('status','rejected')->get();
        $jobsall=$user->jobs;

        //$jobsrevision=jobs::select('*')->where('status','revision')->where('writer',Auth::user()->id)->get();
        $jobsrevision = jobs::QueryW('revision',Auth::user()->id);
        $jobscomplete = jobs::QueryW('complete',Auth::user()->id);
        $jobsrejected = jobs::QueryW('rejected',Auth::user()->id);
        $jobsreceived = jobs::QueryW('received',Auth::user()->id);
        $jobspending = jobs::QueryW('pending',Auth::user()->id);        
        $view
